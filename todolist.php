...
<br>
<div class="hello">To-do lists---- sorted by creation</div>
 


<?php      $idea=$_SESSION['id']; 
$res = mysqli_query($dbb, "SELECT * FROM tod where ide='$idea' and content like 'hi' order by date_created"); ?>
...
<div id="myDIV" class="header" style="padding: 5px 40px;">
  ...
  <form method="post"  >
  <input type="text" class="inp" placeholder="add..." name="addingcontent">
  <button type="submit" class="addBtn" name="addp" value="<?php echo $ro['title']; ?>">Save</button></form>
  <br>
  <a class="button button3" href="server2.php?dl=<?php echo $ro['title']; ?>&iid=<?php echo $_SESSION['id']; ?>">DELETE</a>
</div>
...
<?php if($row['checkd']==0): ?>
<ul id="myUL">
  <li><a href="server2.php?checku=<?php echo $row['content']; ?>&iid=<?php echo $_SESSION['id']; ?>&upl=<?php echo $row['title']; ?>" > <?php echo $row['content'] ?></li></a>
...
<?php else : ?>
  <ul id="myUL" >
  <li class="checked"> <a href="server2.php?uchecku=<?php echo $row['content']; ?>&iid=<?php echo $_SESSION['id']; ?>&upl=<?php echo $row['title']; ?>" ><?php echo $row['content'] ?> </a></li>
